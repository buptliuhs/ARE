...
/**
 * This class contains all information concerning a data source request. The
 * information in this class is used to create a data table and return a
...
public class DataSourceRequest {
    ...
    /**
     * A request header name. Used to determine if the request was sent from the
    ...
    /**
     * The name of the request parameter that specifies the data source
     * parameters.
     */
    ...
    /**
     * Constructor. Useful for debugging and testing. Do not use it for
     * production.
     *
     * @param query
     *            The query.
     ...
     *            The user locale.
     */
    public DataSourceRequest(Query query, DataSourceParameters dsParams,
            ULocale userLocale) {
     ...
     *             In case of an invalid 'tq' or 'tqx' parameter.
     */
     ...
     *
     * Used in case no data source request is available (i.e., because of
     * invalid input). Parses the input 'tq' and 'tqx' strings and, if it fails,
     * uses default values.
     *
     * @param req
     ...
     */
    public static DataSourceRequest getDefaultDataSourceRequest(
        ...
        try {
            ...
            if (dataSourceRequest.dsParams == null) {
                dataSourceRequest.dsParams = DataSourceParameters
    ...
    public static boolean determineSameOrigin(HttpServletRequest req) {
        // We conclude that the request is sent from the same origin if it
        // contains a predefined
        // header that the client application inserted. This is a known way of
        // verifying that
    ...
    /**
     * Creates the <code>Query</code> based on the 'tq' parameter on the given
     * request.
     *
     * @param req
     *            The http servlet request.
     ...
     *             if the 'tq' string is invalid or missing on the request.
     */
    private void createQueryFromRequest(HttpServletRequest req)
            throws InvalidQueryException {
    ...
    /**
     * Creates the <code>DataSourceParameters</code> based on the 'tqx'
     * parameter on the given request.
     *
     * @param req
     *            The http servlet request.
     *
     * @throws DataSourceException
     *             Thrown if the data source parameters or query could not be
     *             parsed.
     */
    private void createDataSourceParametersFromRequest(HttpServletRequest req)
            throws DataSourceException {
        // Set the data source parameters.
        String dataSourceParamsString = req
                .getParameter(DATASOURCE_REQUEST_PARAMETER);

        dsParams = new DataSourceParameters(dataSourceParamsString);
        // For backward compatibility, set the OutputType to be 'jsonp' in case
        // the request is cross
