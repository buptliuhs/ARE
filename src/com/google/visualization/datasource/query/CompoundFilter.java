...
/**
 * A compound filter. This filter is a logical aggregation of other filters.
 * Currently, union (OR) and intersection (AND) are supported. An OR complex
 * filter matches if any of its sub-filters match. An AND complex filter matches
 * if all of its sub-filters match.
 *
 * @author Yonatan B.Y.
 */
public class CompoundFilter extends QueryFilter {
    ...
    /**
     * The available types of CompoundFilter. Currently supported are AndFilter
     * and OrFilter.
     */
    ...
    /**
     * Constructs a compound filter of the given type, with the given
     * sub-filters.
     *
     * @param operator
     *            The type of this compound filter.
     * @param subFilters
     *            The collection of all sub-filters of this compound filter.
     */
    ...
    /**
     * Implements isMatch (from the QueryFilter interface) by recursively
     * calling isMatch on each of the sub-filters, and using the compound filter
     * type to determine the result. Note: 1. This uses short-circuit
     * evaluation, i.e., an OR filter decides on a true result the moment it
     * finds a true result among its sub-filters, without continuing to check
     * the rest of the sub-filters; and dually for an AND filter. 2. The
     * sub-filters are evaluated in order. 3. If the collection of sub-filters
     * is empty, a RuntimeException is thrown.
     *
     * @param table
     ...
     *            The row to check.
     *
     * @return true if this row should be part of the result set, false
     *         otherwise.
     */
    @Override
    ...
    /**
     * Returns a list of all scalarFunctionColumns this filter uses, in this
     * case the union of all the results of getScalarFunctionColumns() of all
     * its sub-filters.
     *
    ...
    public String toQueryString() {
        ...
        // This works because the names in LogicalOperator are exactly the same
        // as
        // in the query language.
        return new StrBuilder().appendWithSeparators(subFilterStrings,
    ...
    public int hashCode() {
        ...
        result = prime * result
                + ((subFilters == null) ? 0 : subFilters.hashCode());
        return result;
    }
