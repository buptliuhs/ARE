...
/**
 * A "title" (identification) of a scalar function column in a pivoted and/or
 * grouped table. What identifies a column is the list of values, one value from
 * each of the pivot columns, and a ScalarFunctionColumn denoting the original
 * column (of the original table columns).
 *
 * @author Liron L.
 */
/* package */class ScalarFunctionColumnTitle {
     ...
     * the values in the pivot-by columns.
     */
    public static final String PIVOT_COLUMNS_SEPARATOR = ",";

    /**
     * When creating the id of the column, this is used as a separator between
     * the list of values and the ScalarFunctionColumn.
     */
     ...
     */
    public static String getColumnDescriptionLabel(DataTable originalTable,
            AbstractColumn column) {
        StringBuilder label = new StringBuilder();
        if (originalTable.containsColumn(column.getId())) {
            label.append(originalTable.getColumnDescription(column.getId())
                    .getLabel());
        } else {
            if (column instanceof AggregationColumn) {
                AggregationColumn aggColumn = (AggregationColumn) column;
                label.append(aggColumn.getAggregationType().getCode())
                        .append(" ")
                        .append(originalTable.getColumnDescription(
                                aggColumn.getAggregatedColumn().getId())
                                .getLabel());
            } else {
                ScalarFunctionColumn scalarFunctionColumn = (ScalarFunctionColumn) column;
                List<AbstractColumn> columns = scalarFunctionColumn
                        .getColumns();
                label.append(
                        scalarFunctionColumn.getFunction().getFunctionName())
                        .append("(");
                for (AbstractColumn abstractColumn : columns) {
                    label.append(getColumnDescriptionLabel(originalTable,
                            abstractColumn));
                }
                label.append(")");
            }
    ...
    public boolean equals(Object o) {
        if (o instanceof ScalarFunctionColumnTitle) {
            ...
            return (values.equals(other.values) && scalarFunctionColumn
                    .equals(other.scalarFunctionColumn));
        }
        return false;
    }
