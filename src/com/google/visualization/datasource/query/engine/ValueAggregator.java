 ...
 * This allows getting the values of: minimum, maximum, sum, count and average
 * for the aggregated set. Each one of these values is available only where
 * appropriate (for instance, you cannot average on text values). The set of
 * values itself is not stored. Only non-null values are considered for
 * aggregation.
 *
 * @author Yoav G.
 */
...
/* package */class ValueAggregator {
     ...
     *
     * @param valueType
     *            The column type of this aggregator. This type defines the type
     *            of all values to be aggregated.
     */
     ...
     *             In case the column type does not support sum.
     */
     ...
     *
     * @throws UnsupportedOperationException
     *             If the column type does not support average.
     */
    ...
    /**
     * Returns a single value. Note: The aggregation of a zero number of rows
     * returns a null value for all aggregation types except from count. The
     * type of Null value is numeric for sum and average and identical to its
     * column values for min and max.
     *
     * @param type
     *            The type of aggregation requested.
     *
    ...
    public Value getValue(AggregationType type) {
        ...
        switch (type) {
        case AVG:
            v = (count != 0) ? new NumberValue(getAverage()) : NumberValue
                    .getNullValue();
            break;
        case COUNT:
        ...
        case MIN:
            ...
            break;
        case SUM:
            v = (count != 0) ? new NumberValue(getSum()) : NumberValue
                    .getNullValue();
            break;
        default:
