...
/**
 * An aggregation node is a node in an aggregation tree. This node holds a
 * value, equal to the value in the corresponding group-by column. It also
 * holds: value aggregators (one for each aggregation column), a reference to
 * its parent in the tree, and a set of references to its children. Each child
 * is associated with a unique value, which corresponds to the value held in the
 * child node. See {@link AggregationTree} for more details.
 *
 * @author Yoav G.
 */
public class AggregationNode {
     ...
     * node (in the aggregation tree), and is used for navigation. Note that the
     * value is the same as the value used as a key to point to this
     * AggregationNode in the parent's {@link #children} map.
     */
    private Value value;

    /**
     * Maps a column id to its aggregator. The column id should belong to the
     * list of aggregation columns.
     */
    private Map<String, ValueAggregator> columnAggregators = Maps.newHashMap();

    /**
     * Maps a value to a child of this node (which is also an aggregation node).
     * ‎The value is the same as the {@link #value} that will be stored in the
     * child, i.e., <code>children.get(X).getValue()</code> should equal
     * <code>X</code>.
     */
     ...
     *            A set of ids of the columns to aggregate (aggregation
     *            columns).
     * @param table
     *            The table.
     */
    public AggregationNode(Set<String> columnsToAggregate, DataTable table) {
        ...
        for (String columnId : columnsToAggregate) {
            columnAggregators.put(columnId, new ValueAggregator(table
                    .getColumnDescription(columnId).getType()));
        }
    }
     ...
     *
     * @param valuesByColumn
     *            Maps a column id to the value that needs be aggregated (for
     *            that column).
     */
    public void aggregate(Map<String, Value> valuesByColumn) {
        for (String columnId : valuesByColumn.keySet()) {
            columnAggregators.get(columnId).aggregate(
                    valuesByColumn.get(columnId));
        }
    }
     ...
     *
     * @return True if this node contains a child (identified by value) and
     *         false otherwise.
     */
     ...
     *
     * @param key
     *            The value defining the new child.
     * @param columnsToAggregate
     *            The ids of the columns to aggregate.
     * @param table
     *            The table.
     */
    public void addChild(Value key, Set<String> columnsToAggregate,
            DataTable table) {
