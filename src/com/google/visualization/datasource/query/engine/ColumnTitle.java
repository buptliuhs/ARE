 ...
 * pivot columns, and a ColumnAggregation denoting on which column (of the
 * original table columns) the aggregation is performed and what type of
 * aggregation (min, max, average, ...) is performed.
 *
 * @author Yonatan B.Y.
 */
/* package */class ColumnTitle {
     ...
     * the values in the pivot columns.
     */
    public static final String PIVOT_COLUMNS_SEPARATOR = ",";

    /**
     * When creating the id of the column, this is used as a separator between
     * the list of values and the column aggregation.
     */
     ...
     * @param isMultiAggregationQuery
     *            Whether this table comes from a query with more than one
     *            aggregation.
     */
    public ColumnTitle(List<Value> values, AggregationColumn aggregationColumn,
            boolean isMultiAggregationQuery) {
        this.values = values;
     ...
     * concatenating the values, and the column aggregation, using the
     * separators PIVOT_COLUMNS_SEPARATOR, PIVOT_AGGREGATION_SEPARATOR,
     * COLUMN_AGGRGATION_TYPE_SEPARATOR.
     *
     * @param originalTable
     *            The original table, from which the original column description
    ...
    public ColumnDescription createColumnDescription(DataTable originalTable) {
        ColumnDescription colDesc = originalTable
                .getColumnDescription(aggregation.getAggregatedColumn().getId());
        return createAggregationColumnDescription(colDesc);
    }

    /**
     * Creates a prefix for a pivoted column id, containing all the values of
    ...
    private String createIdPivotPrefix() {
        ...
        return new StrBuilder()
                .appendWithSeparators(values, PIVOT_COLUMNS_SEPARATOR)
    ...
    private String createLabelPivotPart() {
        ...
        return new StrBuilder().appendWithSeparators(values,
                PIVOT_COLUMNS_SEPARATOR).toString();
    }
     ...
     *
     * @param originalColumnDescription
     *            The original column description.
     *
    ...
    /* package */ColumnDescription createAggregationColumnDescription(
        ...
        String aggregationLabelPart = aggregation.getAggregationType()
                .getCode() + " " + originalColumnDescription.getLabel();
        ...
        if (canUseSameTypeForAggregation(type, aggregationType)) {
            ...
            // Create a new column description if this is a non numeric column
            // and the aggregation
            // type is COUNT.
    ...
    /**
     * Checks whether the aggregation values and column values are of the same
     * type.
     *
     * @param valueType
     *            The type of values in the aggregated column.
