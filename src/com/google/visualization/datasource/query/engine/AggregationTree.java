...
/**
 * An aggregation tree is the logical data structure that represents table
 * grouping information. Each level of the tree (besides the root-level) is
 * associated with an aggregation column. Each node of the tree is associated
 * with a value (of some cell in the column defining its level). Each node
 * contains aggregation information, i.e., information about the minimum,
 * maximum, count, average and sum on the aggregated columns. A leaf node holds
 * this aggregation information for the corresponding "path" of values. For
 * example, if the group-by columns are {Name, Revenue}, then the tree consists
 * of 3 levels: the root level, holding only the root, then a level with nodes
 * representing different values of Name, and then a level with nodes holding
 * different values of Revenue. Then the leaf node which is at the path {"Joe",
 * 100} from the root will hold the aggregation information for all rows in the
 * table for which the value of Name is "Joe" and the value of Revenue is 100.
 * The non-leaf node which is at the path {"Joe"} will contain the aggregation
 * information for all rows in which the name is "Joe", without any
 * consideration of the value of the Revenue column. The root node contains all
 * aggregation information for the entire table.
 *
 * @author Yoav G.
 */
public class AggregationTree {
    ...
    /**
     * The root of the tree is associated with the empty path and aggregates all
     * data rows.
     */
     ...
     *            A set of ids of the columns to aggregate.
     * @param table
     *            The table.
     */
     ...
     *
     * @param valuesToAggregate
     *            Maps column ids to values (to aggregate).
     * @param path
     *            The aggregation path.
     */
    public void aggregate(AggregationPath path,
            Map<String, Value> valuesToAggregate) {
     ...
     *             In case no node lies at the end of the path.
     */
     ...
     */
    private void getPathsToLeavesInternal(AggregationNode node,
            Set<AggregationPath> result) {
        // Perform DFS.
    ...
    /**
     * Returns the path in the aggregation tree from the root to an aggregation
     * node.
     *
     * @param node
