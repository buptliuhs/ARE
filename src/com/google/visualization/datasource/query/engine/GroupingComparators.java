...
/* package */class GroupingComparators {
    ...
    /**
     * Compares a list of values "lexicographically", i.e., if l1 = (x1...xn)
     * and l2 = (y1...ym) then compare(l1, l2) will be compare(xi,yi) when i =
     * argmin_k(compare(xk, yk) != 0). The result is 0 if no such k exists and
     * both lists are of the same size. If the lists are not of the same size
     * and no such k exists, then the longer list is considered to be greater.
     */
     ...
     * ColumnAggregation isn't in the given list, then it shouldn't be compared
     * using this comparator and a runtime exception will be thrown.
     *
     * @param columnAggregations
     *            The list that orders the ColumnAggregations.
     *
     ...
     * list of
     * {@link com.google.visualization.datasource.query.AggregationColumn}s.
     * <p>
     * It compares ColumnTitles by first comparing their lists of (pivot) values
     * "lexicographically" (by using {@code VALUE_LIST_COMPARATOR}) and if these
     * are equal, compares the ColumnAggregation within the title, by simply
     * finding their index in the given list, and comparing the indices, i.e.,
     * two ColumnTitles that have the same pivot values, will compare according
     * to which ColumnTitle's ColumnAggregation comes first in the given list.
     * If a ColumnTitle's ColumnAggregation isn't in the given list, then it
     * shouldn't be compared using this comparator and a runtime exception will
     * be thrown.
     */
    private static class ColumnTitleDynamicComparator implements
            Comparator<ColumnTitle> {
         ...
         *
         * @param aggregations
         *            The list that gives order to the ColumnAggregations.
         */
         ...
         *
         * @param col1
         *            The first ColumnTitle
         * @param col2
         *            The second ColumnTitle
         *
        ...
        public int compare(ColumnTitle col1, ColumnTitle col2) {
            ...
            return aggregationsComparator.compare(col1.aggregation,
                    col2.aggregation);
        }
    }
}
