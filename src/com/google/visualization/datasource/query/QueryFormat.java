...
/**
 * Describes the formatting options of a Query, as passed on the query in the
 * "format" clause.
 *
 * Format can be specified for each column, but does not have to be. If not
 * specified, each column type has a default format.
 *
...
public class QueryFormat {
     ...
     * Log.
     */
    private static final Log log = LogFactory.getLog(QueryFormat.class
            .getName());

    /**
     * A map of all of the columns that require non-default patterns, and the
     * pattern specified for them. Columns with default patterns are not in the
     * map.
     */
    private Map<AbstractColumn, String> columnPatterns;

    /**
     * Default empty constructor, with no patterns (patterns can be added
     * later).
     */
    public QueryFormat() {
    ...
    /**
     * Adds a column pattern. Validates that the column ID is not already
     * specified.
     *
     * @param column
     *            The column to which the pattern is assigned.
     * @param pattern
     *            The assigned pattern.
     *
     * @throws InvalidQueryException
     *             Thrown if the column is already specified.
     */
    public void addPattern(AbstractColumn column, String pattern)
            throws InvalidQueryException {
        if (columnPatterns.keySet().contains(column)) {
            String messageToLogAndUser = "Column [" + column.toString()
                    + "] is " + "specified more than once in FORMAT.";
     ...
     *
     * @param column
     *            The column for which the pattern is required.
     *
     ...
     * functions columns that are inside other scalar function columns (e.g.,
     * sum(year(a), year(b))).
     *
    ...
    public List<ScalarFunctionColumn> getScalarFunctionColumns() {
        ...
        for (AbstractColumn col : columnPatterns.keySet()) {
            for (ScalarFunctionColumn innerCol : col
    ...
    public int hashCode() {
        ...
        int result = 1;
        result = prime * result
                + ((columnPatterns == null) ? 0 : columnPatterns.hashCode());
        return result;
    }
    ...
    /**
     * Returns a string that when fed into the query parser, produces a
     * QueryFormat equal to this one. The string returned does not contain the
     * FORMAT keyword.
     * 
    ...
    public String toQueryString() {
        ...
        for (AbstractColumn col : columnPatterns.keySet()) {
            ...
            stringList.add(col.toQueryString() + " "
                    + Query.stringToQueryStringLiteral(pattern));
        }
