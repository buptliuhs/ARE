...
/**
 * Describes the labels of a Query, as passed in the query under the "label"
 * clause.
 *
 * A label can optionally be specified for each column. If not specified, each
 * column type has a default label.
 *
 * @author Itai R.
 */
public class QueryLabels {
     ...
     * Log.
     */
    private static final Log log = LogFactory.getLog(QueryLabels.class
            .getName());

    /**
     * A map of all of the columns that require non-default labels, and the
     * label specified for them. Columns with default labels are not in the map.
     */
    ...
    /**
     * Adds a column label. Validates that the column ID is not already
     * specified.
     *
     * @param column
     *            The column to which the label is assigned.
     * @param label
     *            The assigned label.
     *
     ...
     */
    public void addLabel(AbstractColumn column, String label)
            throws InvalidQueryException {
        if (columnLabels.keySet().contains(column)) {
            String messageToLogAndUser = "Column [" + column.toString()
                    + "] is " + "specified more than once in LABEL.";
     ...
     *
     * @param column
     *            The column for which the label is required.
     *
    ...
    public int hashCode() {
        ...
        int result = 1;
        result = prime * result
                + ((columnLabels == null) ? 0 : columnLabels.hashCode());
        return result;
    }
     ...
     * QueryLabels equal to this one. The string returned does not contain the
     * LABEL keyword.
     * 
    ...
    public String toQueryString() {
        ...
        for (AbstractColumn col : columnLabels.keySet()) {
            ...
            stringList.add(col.toQueryString() + " "
                    + Query.stringToQueryStringLiteral(label));
        }
