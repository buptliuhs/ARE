...
public class AggregationColumn extends AbstractColumn {
    /**
     * When creating the ID of the column, this separates the column on which
     * aggregation is performed and the type of aggregation performed on it.
     */
    ...
    /**
     * Returns a list of all simple columns. In this case, returns an empty
     * list.
     *
    ...
    /**
     * Checks whether it makes sense to have the aggregation type on the
     * aggregated column. The type of the column is taken from the given table
     * description. Throws a column exception if the column is invalid.
     *
     * @param dataTable
     *            The data table.
     *
    ...
    public void validateColumn(DataTable dataTable)
            throws InvalidQueryException {
        ValueType valueType = dataTable.getColumnDescription(
                aggregatedColumn.getId()).getType();
        ...
        case SUM:
            if (valueType != ValueType.NUMBER) {
                throw new InvalidQueryException(
                        MessagesEnum.AVG_SUM_ONLY_NUMERIC
                                .getMessage(userLocale));
            }
            break;
        default:
            throw new RuntimeException(
                    MessagesEnum.INVALID_AGG_TYPE.getMessageWithArgs(
    ...
    public ValueType getValueType(DataTable dataTable) {
        ValueType valueType;
        ValueType originalValueType = dataTable.getColumnDescription(
                aggregatedColumn.getId()).getType();
        switch (aggregationType) {
        case COUNT:
        ...
        default:
            throw new RuntimeException(
                    MessagesEnum.INVALID_AGG_TYPE.getMessageWithArgs(
                            dataTable.getLocaleForUserMessages(),
