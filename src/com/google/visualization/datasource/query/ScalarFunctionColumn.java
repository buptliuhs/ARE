...
/**
 * A scalar function column (e.g. year(Date1)). The values in a scalar function
 * column are the result of executing the function on the columns that are given
 * as parameters, so for example year(Date1) column values will have the year of
 * the corresponding value in Date1 column.
 *
 * @author Liron L.
 */
public class ScalarFunctionColumn extends AbstractColumn {
    ...
    /**
     * When creating the ID of the column, this is used as a separator between
     * the columns on which the function is performed.
     */
     ...
     *
     * @param columns
     *            The list of columns on which the function is performed.
    ...
    /**
     * Returns the ID of the scalar function column. THe ID is constructed from
     * the function's name and the IDs of the inner columns on which the
    ...
    public String getId() {
        ...
        return new StrBuilder(scalarFunction.getFunctionName())
                .append(COLUMN_FUNCTION_TYPE_SEPARATOR)
                .appendWithSeparators(colIds, COLUMN_COLUMN_SEPARATOR)
                .toString();
    }
     ...
     * lookup. Otherwise, recursively gets the inner column values and uses them
     * to evaluate the value and create a cell based on that value. The base of
     * the recursion is a simple column, aggregation column or another scalar
     * function column that exists in the column lookup (i.e., its value was
     * already calculated).
     *
     * @param row
     *            The given row.
    ...
    public TableCell getCell(ColumnLookup lookup, TableRow row) {
        ...
        // If the given column lookup does not contain this column, get the
        // inner
        // column values of this column and use them as parameters to evaluate
        // the
        // scalar function value in the given row.
        List<Value> functionParameters = Lists.newArrayListWithCapacity(columns
                .size());
        for (AbstractColumn column : columns) {
    ...
    /**
     * Returns a list of all simple columns. This includes simple columns that
     * are inside scalar function columns (e.g, year(a1)), but does not include
     * simple columns that are inside aggregation columns (e.g., sum(a1)).
     *
     * @return A list of all simple columns.
     */
    @Override
    public List<SimpleColumn> getAllSimpleColumns() {
        List<SimpleColumn> simpleColumns = Lists
                .newArrayListWithCapacity(columns.size());
        for (AbstractColumn column : columns) {
    ...
    public List<AggregationColumn> getAllAggregationColumns() {
        List<AggregationColumn> aggregationColumns = Lists
                .newArrayListWithCapacity(columns.size());
        for (AbstractColumn column : columns) {
    ...
    /**
     * Returns a list of all scalar function columns. Returns itself and other
     * inner scalar function columns (if there are any). e.g., the column
     * max(year(a1), year(a2)) will return the 3 columns: max(year(a1),
     * year(a2)), year(a1), year(a2).
     *
     * @return A list of all scalar function columns.
     */
    @Override
    public List<ScalarFunctionColumn> getAllScalarFunctionColumns() {
        List<ScalarFunctionColumn> scalarFunctionColumns = Lists
                .newArrayList(this);
        for (AbstractColumn column : columns) {
    ...
    /**
     * Checks that the column is valid. Checks the scalar function matches its
     * arguments (inner columns) and all its inner columns are valid too. Throws
     * a ColumnException if the scalar function has invalid arguments.
     *
     * @param dataTable
    ...
    public void validateColumn(DataTable dataTable)
        ...
        // Throws an InvalidColumnException when the function arguments types
        // are
        // invalid.
    ...
    /**
     * Returns the value type of the column after evaluating the scalar
     * function. e.g., the value type of year(date1) is NUMBER.
     *
     * @param dataTable
     *            The table description.
     *
    ...
    public String toString() {
        ...
        return new StrBuilder(scalarFunction.getFunctionName()).append("(")
                .appendWithSeparators(colNames, COLUMN_COLUMN_SEPARATOR)
                .append(")").toString();
    }
}
