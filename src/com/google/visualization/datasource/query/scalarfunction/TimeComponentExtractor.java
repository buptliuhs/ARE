...
/**
 * A time component extractor. This class encompasses all the unary scalar
 * functions that extract a time component from a date/datetime/timeofday value.
 * e.g., year, month, second.
 *
 * @author Liron L.
 */
public class TimeComponentExtractor implements ScalarFunction {
    ...
    public static enum TimeComponent {
        YEAR("year"), MONTH("month"), WEEK("week"), DAY("day"), HOUR("hour"), MINUTE(
                "minute"), SECOND("second"), MILLISECOND("millisecond"), QUARTER(
                "quarter"),
    ...
    /**
     * A mapping of a TimeComponent to a TimeComponentExtractor that extracts
     * this TimeComponent. This is used to keep a single instance of every type
     * of extractor.
     */
    ...
    /**
     * This static block builds the timeComponentsPool that maps TimeComponents
     * to TimeComponentExtractors that extract this TimeComponent.
     */
    static {
        timeComponentsPool = Maps.newHashMap();
        for (TimeComponent component : TimeComponent.values()) {
            timeComponentsPool.put(component, new TimeComponentExtractor(
                    component));
        }
    }
    ...
    /**
     * Returns the name of this scalar function, in this case the name of the
     * TimeComponentExtractor.
     *
     * @return the name of this scalar function, in this case the name of the
     *         TimeComponentExtractor.
     */
     ...
     * Date/DateTime/TimeOfDay value. The method does not validate the
     * parameters, the user must check the parameters before calling this
     * method.
     *
     * @param values
     *            A list of the values on which the scalar function is
     *            performed.
     *
    ...
    public Value evaluate(List<Value> values) {
        ...
        case QUARTER:
            ...
            break;
        case DAY_OF_WEEK:
            GregorianCalendar calendar = (GregorianCalendar) ((valueType == ValueType.DATE) ? ((DateValue) value)
                    .getObjectToFormat() : ((DateTimeValue) value)
                    .getObjectToFormat());
            component = calendar.get(GregorianCalendar.DAY_OF_WEEK);
            break;
        default:
            // should not get here since we assume that the given values are
            // valid.
     ...
     *
     * @param types
     *            A list of the types of the scalar function parameters.
     *
     ...
     * that its type is DATE or DATETIME if the timeComponent to extract is
     * year, month or day, or that it is DATETIME or TIMEOFDAY if the
     * timeComponent is hour, minute, second or millisecond. Throws a
    ...
    public void validateParameters(List<ValueType> types)
        ...
        if (types.size() != 1) {
            throw new InvalidQueryException("Number of parameters for "
                    + timeComponent.getName() + "function is wrong: "
                    + types.size());
        }
        switch (timeComponent) {
        case YEAR:
        ...
        case DAY_OF_WEEK:
            if ((types.get(0) != ValueType.DATE)
                ...
                throw new InvalidQueryException("Can't perform the function "
                        + timeComponent.getName()
                        + " on a column that is not a Date or"
                        + " a DateTime column");
            }
            break;
        case HOUR:
