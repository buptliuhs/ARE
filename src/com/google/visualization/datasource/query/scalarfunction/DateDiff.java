...
/**
 * The binary scalar function datediff(). Returns the difference in days between
 * two dates or date time values.
 *
 * @author Liron L.
 */
public class DateDiff implements ScalarFunction {
    ...
    /**
     * A private constructor, to prevent instantiation other than by the
     * singleton.
     */
    private DateDiff() {
    }
    ...
    /**
     * Executes this scalar function on the given values. Returns values[0] -
     * values[1] expressed as a number value denoting the number of days from
     * one date to the other. Both values can be of type date or date-time. Only
     * the date parts of date-time values are used in the calculation. Thus the
     * returned number is always an integer. The method does not validate the
     * parameters, the user must check the parameters before calling this
     * method.
     *
     * @param values
     *            A list of values on which the scalar function will be
     *            performed.
     *
     * @return Value holding the difference, in whole days, between the two
     *         given Date/DateTime values, or a null value (of type number) if
    ...
    public Value evaluate(List<Value> values) {
        ...
        return new NumberValue(calendar.fieldDifference(firstDate,
                Calendar.DATE));
    }

    /**
     * Converts the given value to date. The value must be of type date or
     * datetime.
     *
     * @param value
     *            The given value.
    ...
    /**
     * Validates that there are only 2 parameters given for this function, and
     * that their types are either DATE or DATETIME. Throws a
     * ScalarFunctionException if the parameters are invalid.
     *
     * @param types
     *            A list with parameters types.
     *
