...
/**
 * A 0-ary function that returns the current datetime. The return type is always
 * DateTime.
 *
 * @author Liron L.
 */
public class CurrentDateTime implements ScalarFunction {
    ...
    /**
     * Private constructor, to prevent instantiation other than by the
     * singleton.
     */
    ...
    public void validateParameters(List<ValueType> types)
            throws InvalidQueryException {
        if (types.size() != 0) {
            throw new InvalidQueryException("The " + FUNCTION_NAME
                    + " function should not get " + "any parameters");
        }
    }
