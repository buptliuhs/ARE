...
/**
 * The unary scalar function toDate(). Transforms the given value to date value.
 * If the given value is already a date, it's trivial. If the given value is a
 * date-time, it extracts its date part. If the given value is number, it treats
 * that number as the number of milliseconds since the Epoch, and returns the
 * corresponding date.
 *
 * @author Liron L.
 */
public class ToDate implements ScalarFunction {
    ...
    /**
     * Executes the scalar function toDate() on the given values. The list is
     * expected to be of length 1, and the value in it should be of an
     * appropiate type. The method does not validate the parameters, the user
     * must check the parameters before calling this method.
     *
     * @param values
     *            A list with the values that the scalar function is performed
     *            on them.
     *
    ...
    public Value evaluate(List<Value> values) {
        ...
        case DATETIME:
            dateValue = new DateValue(
                    (GregorianCalendar) (((DateTimeValue) value)
                            .getObjectToFormat()));
            break;
        case NUMBER:
        ...
        default:// Should never get here.
            throw new RuntimeException("Value type was not found: "
                    + value.getType());
        }
        return dateValue;
    }
