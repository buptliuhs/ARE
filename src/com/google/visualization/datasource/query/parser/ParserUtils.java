...
/**
 * A utility class for the QueryParser. The functions here are called from the
 * .jj file.
 *
 * Note on errors: Since this class handles a user generated query, all errors
 * detected cause both logging, and an exception that is thrown to the user.
 *
 * @author Yonatan B.Y.
 */
/* package */final class ParserUtils {
    ...
    /**
     * The message that should be in the exception thrown when parsing an
     * invalid date string, parameterized by the erroneous string itself.
     */
    ...
    /**
     * The message that should be in the exception thrown when parsing an
     * invalid timeofday string, parameterized by the erroneous string itself.
     */
    ...
    /**
     * The message that should be in the exception thrown when parsing an
     * invalid datetime string, parameterized by the erroneous string itself.
     */
    private static final String dateTimeMessage = "Invalid datetime literal [%1$s]. Datetime literals should "
            + " be of form yyyy-MM-dd HH:mm:ss[.SSS]";

    /**
     * Parses a string into a date value, for the query parser. The dates parsed
     * are always in the format: yyyy-MM-dd.
     * 
     * @param s
     *            The string to parse.
     * 
     ...
     *             If the string is not parse-able as a date of format
     *             yyyy-MM-dd.
     */
    ...
    /**
     * Parses a string into a time-of-day value, for the query parser. The
     * values parsed are always in the format: HH:mm:ss[.SSS].
     * 
     * @param s
     *            The string to parse.
     * 
     ...
     * 
     * @throws InvalidQueryException
     *             If the string can not be parsed into a time-of-day of format
     *             HH:mm:ss[.SSS].
     */
    ...
    /**
     * Parses a string into a date-time value, for the query parser. The values
     * parsed are always in the format: yyyy-MM-dd HH:mm:ss[.SSS].
     * 
     * @param s
     *            The string to parse.
     * 
     ...
     * 
     * @throws InvalidQueryException
     *             If the string can not be parsed into a date-time of format
    ...
    /**
     * Strips the first and last characters from a string. Throws a runtime
     * exception if the string is less than 2 characters long. Used for
     * stripping quotes (whether single, double, or back-quotes) from, for
     * example, "foo", 'bar', and `baz`.
     *
     * @param s
     *            The string from which to strip the quotes.
     *
