 ...
 *
 * An instance of this class is the result of a request to a data source. A
 * <code>DataTable</code> can be rendered in many ways: JSON, HTML, CSV (see
 * {@link com.google.visualization.datasource.render}), and can be manipulated
 * using queries (see {@link com.google.visualization.datasource.query.Query}
 * and {@link com.google.visualization.datasource.query.engine.QueryEngine}).
 * 
 * A table contains any number of typed columns (see (@link ColumnDescription})
 * each with an id and a label, and any number of rows. Each row must have as
 * many cells as there are columns in the table, and the types of values in the
 * cells must match the types of the columns. Each cell contains, as well as the
 * typed value, a formatted value of type string, used for display purposes.
 * Also, you can use the custom properties mechanism to hold any other data you
 * require. There are custom properties on every cell, row, column, and on the
 * entire table.
 *
 * @author Yoah B.D.
 */
public class DataTable {
    ...
    /**
     * Adds a single row to the end of the result. Throws a
     * TypeMismatchException if the row's cells do not match the current
     * columns. If the row is too short, i.e., has too few cells, then the
     ...
     *             Thrown if the values in the cells do not match the columns.
     */
    public void addRow(TableRow row) throws TypeMismatchException {
        List<TableCell> cells = row.getCells();
        if (cells.size() > columns.size()) {
            throw new TypeMismatchException(
                    "Row has too many cells. Should be at most of size: "
                            + columns.size());
        }
        for (int i = 0; i < cells.size(); i++) {
            if (cells.get(i).getType() != columns.get(i).getType()) {
                throw new TypeMismatchException(
                        "Cell type does not match column type, at index: " + i
                                + ". Should be of type: "
                                + columns.get(i).getType().toString());
            }
        }
        for (int i = cells.size(); i < columns.size(); i++) {
            row.addCell(new TableCell(Value.getNullValueFromValueType(columns
                    .get(i).getType())));
        }
     ...
     *            The row values.
     * @throws TypeMismatchException
     *             Thrown if a value does not match its corresponding column.
     */
     ...
     *            The row collection.
     */
    public void addRows(Collection<TableRow> rowsToAdd)
            throws TypeMismatchException {
     ...
     * Note: This is the most naive implementation, that for each request to
     * this method just creates a new List of the needed cells.
     *
     * @param columnIndex
    ...
    public List<TableCell> getColumnCells(int columnIndex) {
        List<TableCell> colCells = Lists
                .newArrayListWithCapacity(getNumberOfRows());
    ...
    public void addColumn(ColumnDescription columnDescription) {
        ...
        if (columnIndexById.containsKey(columnId)) {
            throw new RuntimeException("Column Id [" + columnId
                    + "] already in table description");
        }
        ...
        for (TableRow row : rows) {
            row.addCell(new TableCell(Value
                    .getNullValueFromValueType(columnDescription.getType())));
        }
    }
     ...
     *
     * @param columnId
     *            The id of the requested column.
     *
     * @return The list of all cells of the requested column.
     */
    ...
    /**
     * Replaces an existing cell at a specified position in this table with the
     * specified cell. The value type of the new cell must match the type of the
     * existing one.
     * 
     * @param rowIndex
     *            The row index.
     ...
     *            The column index.
     * @param cell
     *            The cell to be stored at the specified position.
     * 
     * @return The cell that was replaced.
     * 
     * @throws TypeMismatchException
     *             Thrown if the new cell value type doesn't match the table
     *             column value type.
        ...
        if (!row.getCell(colIndex).getType().equals(cell.getType())) {
            throw new TypeMismatchException(
                    "New cell value type does not match expected value type."
                            + " Expected type: "
                            + row.getCell(colIndex).getType() + " but was: "
    ...
    /**
     * Returns the list of warnings in this table. The list returned is
     * immutable.
     *
     ...
     * The cells are sorted according to the given comparator in ascending
     * order.
     *
     * @param columnIndex
     *            The index of the required column.
     * @param comparator
     *            A Comparator for TableCells.
     *
    ...
    public List<TableCell> getColumnDistinctCellsSorted(int columnIndex,
        ...
        // Return the list of distinct Table cells sorted (based on the
        // comparator).
     ...
     *            The index of the requested column.
     *
     * @return An ordered list of all the distinct values of a single column.
     */
    ...
    /**
     * Returns a string representation of the data table. Useful mainly for
     * debugging.
     *
