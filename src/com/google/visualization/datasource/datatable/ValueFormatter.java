...
/**
 * Formats a {@link Value}, or parses a string to create a {@link Value}. An
 * instance of this class can be created using the
 * {@link #createFromPattern(ValueType, String, ULocale)} method, and can then
 * use the format and/or parse.
 *
 * The class also supplies a set of default patterns per {@link ValueType}. The
 * default patterns can be used for parsing/formatting values when there is no
 * specified pattern. Otherwise, create a class instance by specifying a pattern
 * and locale.
 *
...
public class ValueFormatter {
    ...
    /**
     * Private constructor that constructs an instance of this class from a
     * UFormat. Use {@link #createFromPattern(ValueType, String, ULocale)} to
     * create an instance.
     */
    private ValueFormatter(String pattern, UFormat uFormat, ValueType type,
            ULocale locale) {
    ...
    /**
     * Creates a formatter for the given value type with the given pattern
     * string and locale. If the pattern is illegal returns null. If pattern is
     * null, uses the first default pattern for the given type. If ulocale is
     * null, uses the default locale return by
     * {@code LocaleUtil#getDefaultLocale}.
     *
     * @param type
     *            The column value type.
     * @param pattern
     *            The string pattern representing the formatter pattern.
     * @param locale
     *            The ULocale of the formatter.
     *
     * @return A formatter for the given type, pattern and locale, or null if
     *         the pattern is illegal.
     */
    public static ValueFormatter createFromPattern(ValueType type,
            String pattern, ULocale locale) {
        ...
        // For whichever formatter is created, try to format some arbitrary
        // value, and see if an
        // exception was thrown. If it was thrown, conclude the pattern was
        // illegal, and return null.
        try {
            switch (type) {
            case BOOLEAN:
            ...
            case DATE:
                ...
                ((SimpleDateFormat) uFormat).setTimeZone(TimeZone
                        .getTimeZone("GMT"));
            ...
            case TIMEOFDAY:
                ...
                ((SimpleDateFormat) uFormat).setTimeZone(TimeZone
                        .getTimeZone("GMT"));
                uFormat.format(new TimeOfDayValue(2, 59, 12, 123)
                        .getObjectToFormat());
                break;
            case DATETIME:
                uFormat = new SimpleDateFormat(pattern, locale);
                ((SimpleDateFormat) uFormat).setTimeZone(TimeZone
                        .getTimeZone("GMT"));
                uFormat.format(new DateTimeValue(1995, 7, 3, 2, 59, 12, 123)
                        .getObjectToFormat());
                break;
            case NUMBER:
    ...
    /**
     * Creates a default formatter for the specified value type and locale. If
     * locale is null, uses the default locale returned by
     * {@code LocaleUtil#getDefaultLocale}.
     *
     * @param type
     *            The value type.
     * @param locale
     *            The data table locale.
     *
    ...
    /**
     * Creates default formatters for all the value types for the specified
     * locale. Returns a map of default formatters by type. The map can be used
     * for iterating over a data table and parsing/formatting its values.
     *
     * @param locale
     *            The data table locale.
     *
     ...
     */
    public static Map<ValueType, ValueFormatter> createDefaultFormatters(
            ULocale locale) {
    ...
    /**
     * Creates the corresponding {@code Value} from the given string. If parsing
     * fails, returns a NULL_VALUE for the specified type. For example, if
     * val="3" and type=ValueType.Number, then after successfully parsing the
     * string "3" into the double 3.0 a new NumberValue would be returned with
     * an internal double value = 3.
     *
     * @param val
     *            The string to parse.
     *
     * @return A corresponding {@code Value} for the given string. If parsing
     *         fails the value would be a NULL_VALUE of the correct
     *         {@code ValueType}.
     */
     ...
     *             if val cannot be parsed into a boolean value.
     */
     ...
     *
     * @throws ParseException
     *             If val cannot be parsed into a number value.
     */
     ...
     *
     * @throws ParseException
     *             If val cannot be parsed into a date.
     */
