...
/**
 * A value of type date-time. Used to represent a specific day in a given year
 * as well as a specific time during that day. This differs from
 * {@link DateValue}, which represents only a specific day in a given year.
 * DateTime is represented internally by a GregorianCalendar. The calendar is
 * immutable and is kept only for validation of the input.
 *
 * @author Hillel M.
 */
public class DateTimeValue extends Value {
    ...
    /**
     * Underlying GregorianCalendar. The calendar is immutable and is set only
     * in the constructor.
     */
    private GregorianCalendar calendar;

    /**
     * Stores the hash code for this DateTime. Do not use GregorianCalendar hash
     * code since it does not consider time values and it might trigger internal
     * changes in the calendar values. The hashCode of NULL_VALUE is zero.
     */
    ...
    /**
     * Creates a new DateTime value. The input is checked using a gregorian
     * calendar. Note this uses the java convention for months: January = 0,
     * ..., December = 11.
     *
     * @param year
     *            The year.
     ...
     *             Thrown if one of the parameters is illegal.
     */
    public DateTimeValue(int year, int month, int dayOfMonth, int hours,
        ...
        // Check input.
        // A RunTimeException is thrown here since it is very unusual for
        // structured
        // data to be incorrect.
        if ((getYear() != year) || (getMonth() != month)
                || (getDayOfMonth() != dayOfMonth) || (getHourOfDay() != hours)
                || (getMinute() != minutes) || (getSecond() != seconds)
                || (getMillisecond() != milliseconds)) {
            throw new IllegalArgumentException("Invalid java date "
                    + "(yyyy-MM-dd hh:mm:ss.S): " + year + '-' + month + '-'
    ...
    /**
     * Creates a new instance based on the given {@code GregorianCalendar}. The
     * given calendar's time zone must be set to "GMT" as a precondition to use
     * this constructor. Note: The date time values: year, month, dayOfMonth,
     * hour, minute, second and millisecond correspond to the values returned by
     * calendar.get(field) of the given calendar.
     *
     * @param calendar
     *            A Gregorian calendar on which to base this instance.
     *
     * @throws IllegalArgumentException
     *             When calendar time zone is not set to GMT.
     */
    ...
    public String toString() {
        ...
        String result = String.format(
                "%1$d-%2$02d-%3$02d %4$02d:%5$02d:%6$02d", getYear(),
                getMonth() + 1, getDayOfMonth(), getHourOfDay(), getMinute(),
                getSecond());
        if (getMillisecond() > 0) {
     ...
     *             Thrown when this Value is NULL_VALUE.
     */
