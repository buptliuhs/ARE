...
/**
 * An abstract value of a single cell in a table. Acts as a base class for all
 * the typed value classes. A value should be an immutable class, because it is
 * reference-copied when cloning complex objects like rows or whole tables.
 *
 * @author Itai R.
 */
public abstract class Value implements Comparable<Value> {
    ...
    /**
     * Checks if this object is equal to the given object. Note that no instance
     * can be equal to null and two instances of different classes are not
     * equal.
     *
     * @param o
    ...
    /**
     * Uses an ibm.icu.text.UFormat instance to format Values. The following
     * method returns an object representing this Value that a UFormat can
     * format.
     *
    ...
    public final String toQueryString() {
        if (isNull()) {
            throw new RuntimeException(
                    "Cannot run toQueryString() on a null value.");
        }
    ...
    /**
     * Returns a comparator that compares values according to a given locale (in
     * case of text values).
     *
     * @param ulocale
     *            The ULocale defining the order relation of text values.
     *
     * @return A comparator that compares values according to a given locale (in
     *         case of text values).
     */
    public static Comparator<Value> getLocalizedComparator(final ULocale ulocale) {
        return new Comparator<Value>() {
            private Comparator<TextValue> textValueComparator = TextValue
                    .getTextLocalizedComparator(ulocale);
