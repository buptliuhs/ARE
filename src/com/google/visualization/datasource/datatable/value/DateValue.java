...
/**
 * A value of type date. Used to represent a specific day in a given year. This
 * differs from {@link DateTimeValue}, which represents a specific day in a
 * given year as well as a specific time during that day. Date is represented
 * internally by three values: year, month and dayOfMonth. This class stores
 * only legitimate dates where the validation is done using
...
public class DateValue extends Value {
    ...
    /**
     * Underlying value: month. Note we use the java convention for months, this
     * is: January = 0, February = 1, ..., December = 11.
     */
    private int month;
    ...
    /**
     * Creates a new date value. The input is checked using a GregorianCalendar.
     * Note that we use java convention for months: January = 0, ..., December =
     * 11.
     *
     * @param year
     *            The year.
    ...
    public DateValue(int year, int month, int dayOfMonth) {
        ...
        if ((calendar.get(GregorianCalendar.YEAR) != year)
            ...
            throw new IllegalArgumentException(
                    "Invalid java date (yyyy-MM-dd): " + year + '-' + month
                            + '-' + dayOfMonth);
        }
     ...
     * this constructor. Note: The date values: year, month, dayOfMonth
     * correspond to the values returned by calendar.get(field) of the given
     * calendar.
     *
     * @param calendar
     *            A gregorian to extract this instance values: year, month and
     *            dayOfMonth.
     *
     * @throws IllegalArgumentException
