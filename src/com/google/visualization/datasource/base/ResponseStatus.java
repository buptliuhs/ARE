...
/**
 * A response status holds three parameters: 1) The response type (Ok, Warning
 * or Error). 2) The response reason type. 3) A string with an error message to
 * the user.
 *
 * @author Hillel M.
 */
public class ResponseStatus {
    ...
    /**
     * Constructs a response status object. This object contains a status type,
     * reason type, and a message that is sent to the user.
     *
     * @param statusType
     *            The response status type.
     * @param reasonType
     ...
     * @param description
     *            A message to be passed to the user.
     */
    public ResponseStatus(StatusType statusType, ReasonType reasonType,
            String description) {
    ...
    public static ResponseStatus createResponseStatus(DataSourceException dse) {
        return new ResponseStatus(StatusType.ERROR, dse.getReasonType(),
                dse.getMessageToUser());
    }

    /**
     * Gets a modified response status in case of <code>ReasonType</code>
     * #USER_NOT_AUTHENTICATED by adding a sign in html link for the given url.
     * If no url is provided in the <code>ResponseStatus</code># no change is
     * made.
     *
     * @param responseStatus
     *            The response status.
     *
     * @return The modified response status if modified, or else the original
     *         response status.
     */
    public static ResponseStatus getModifiedResponseStatus(
            ResponseStatus responseStatus) {

        String signInString = LocaleUtil.getLocalizedMessageFromBundle(
                "com.google.visualization.datasource.base.ErrorMessages",
                SIGN_IN_MESSAGE_KEY, null);
        if (responseStatus.getReasonType() == ReasonType.USER_NOT_AUTHENTICATED) {
            ...
            if (!msg.contains(" ")
                ...
                // The description is assumed to be a link to sign in page,
                // transform the message into
                // an html snippet of a link.
                StringBuilder sb = new StringBuilder(
                        "<a target=\"_blank\" href=\"").append(msg)
                        .append("\">").append(signInString).append("</a>");
