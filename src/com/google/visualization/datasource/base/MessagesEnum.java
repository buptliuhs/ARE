...
/**
 * An enum for messages used in the DataSource library. Each element references
 * a message in {@code ErrorMessages}. For example all the error messages
 * produced in query parsing are listed here.
 *
 * @author Hillel M.
 */
public enum MessagesEnum {
    ...
    /**
     * Column should be added to GROUP BY, removed from SELECT, or aggregated in
     * SELECT.
     * 
     * @param column
     *            id
    ...
    /**
     * Column which appears in ORDER BY, must be in SELECT as well, because
     * SELECT contains aggregated columns.
     */
     ...
     */
    private static final Map<MessagesEnum, String> QUERY_ERROR_TO_MESSAGE = Maps
            .newEnumMap(MessagesEnum.class);
    ...
    public String getMessageWithArgs(ULocale ulocale, String... args) {
        ...
        return LocaleUtil.getLocalizedMessageFromBundleWithArguments(
                ...
                QUERY_ERROR_TO_MESSAGE.get(this), args, locale);
    }
    ...
    public String getMessage(ULocale ulocale) {
        ...
        return LocaleUtil.getLocalizedMessageFromBundle(
                "com.google.visualization.datasource.base.ErrorMessages",
                QUERY_ERROR_TO_MESSAGE.get(this), locale);
    }
}
