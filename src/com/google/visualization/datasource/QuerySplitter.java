...
/**
 * A utility class for splitting the user query into a data source query and a
 * completion query. The data source query is executed by the data source, the
 * completion query is then executed by the query engine. The splitting is
 * performed based on the capabilities that the data source declares it can
 * handle.
 *
 * @author Yonatan B.Y.
 *
...
public final class QuerySplitter {
    ...
    /**
     * Split the query into a data source query and completion query. The data
     * source query runs first directly on the underlying data. The completion
     * query is run by
     * {@link com.google.visualization.datasource.query.engine.QueryEngine}
     * engine on the result of the data source query.
     *
     * @param query
     *            The <code>Query</code> to split.
     * @param capabilities
     *            The capabilities supported by the data source.
     *
     * @return A split query.
     *
     * @throws DataSourceException
     *             Thrown if the capabilities are not supported.
     */
    public static QueryPair splitQuery(Query query, Capabilities capabilities)
        ...
        throw new DataSourceException(ReasonType.NOT_SUPPORTED,
                "Capabilities not supported.");
    }

    /**
     * Splits the query for a data source with capabilities ALL. In this case,
     * the original query is copied to the data source query and the original
     * query is empty.
     *
     * @param query
     *            The query to split.
    ...
    /**
     * Splits the query for a data source with capabilities NONE. In this case,
     * the original query is copied to the completionQuery and the data source
     * query is empty. Furthermore, the data source query is assigned null and
     * shouldn't be used or referenced by the data source.
     *
     * @param query
     *            The query to split.
     *
    ...
    /**
     * Splits the query for a data source with capabilities SQL. If the query
     * contains scalar functions, then the query is split as if data source
     * capabilities are NONE. If the query does not contain scalar functions,
     * then the data source query contains most of the operations. Because SQL
     * cannot handle pivoting, special care needs to be taken if the query
     * includes a pivot operation. The aggregation operation required for
     * pivoting is passed to the data source query. We make use of this, with
     * some implementation tricks. See implementation comments.
     *
     * @param query
     *            The original query.
     *
    ...
    private static QueryPair splitSQL(Query query) {
        ...
        // - Queries with pivot that also contain labels or formatting on
        // aggregation columns.
        if (!query.getAllScalarFunctionsColumns().isEmpty()
                || (query.hasPivot() && ((query.hasUserFormatOptions() && !query
                        .getUserFormatOptions().getAggregationColumns()
                        .isEmpty()) || (query.hasLabels() && !query.getLabels()
        ...
        if (query.hasPivot()) {
            // Make the pivot columns additional grouping columns, and handle
            // the
            ...
            // Let the data source group by all grouping/pivoting columns, and
            // let it
            // select all selection/pivoting columns, e.g., SELECT A, max(B)
            // GROUP BY A PIVOT C turns
            ...
            if (dataSourceQuery.hasGroup()) {
                // Same logic applies here, no calculated columns and no
                // aggregations.
            ...
            if (dataSourceQuery.hasSelection()) {
                newSelectionColumns.addAll(dataSourceQuery.getSelection()
                        .getColumns());
            }
            ...
            // Build the completion query to group by the grouping columns.
            // Because an aggregation is
            // required, make a dummy aggregation on the original column by
            // which the aggregation is
            // required.
            // This original column must be unique for a given set of values for
            // the grouping/pivoting
            // columns so any aggregation operation out of MIN, MAX, AVG will
            // return the value
            // itself and will not aggregate anything. The example from before,
            // SELECT A, max(B) GROUP BY A PIVOT C turns into SELECT A,
            // min(max-B) GROUP BY A PIVOT C

            completionQuery.copyFrom(query);
            completionQuery.setFilter(null);

            QuerySelection completionSelection = new QuerySelection();
            List<AbstractColumn> originalSelectedColumns = query.getSelection()
                    .getColumns();
            for (int i = 0; i < originalSelectedColumns.size(); i++) {
                ...
                } else { // Must be an aggregation column if doesn't appear in
                         // the grouping.
                    // The id here is the id generated by the data source for
                    // the column containing
                    // the aggregated data, e.g., max-B.
                    String id = column.getId();
                    // MIN is chosen arbitrarily, because there will be exactly
                    // one.
                    completionSelection.addColumn(new AggregationColumn(
                            new SimpleColumn(id), AggregationType.MIN));
                }
            }

            completionQuery.setSelection(completionSelection);
        } else {
            // When there is no pivoting, sql does everything (except skipping,
            // options, labels, format).
            dataSourceQuery.copyFrom(query);
            dataSourceQuery.setOptions(null);
            completionQuery.setOptions(query.getOptions());
            try {
                // If there is skipping pagination should be done in the
                // completion query
                ...
                if (query.hasLabels()) {
                    ...
                    for (AbstractColumn column : labels.getColumns()) {
                        newLabels.addLabel(new SimpleColumn(column.getId()),
                                labels.getLabel(column));
                    }
                ...
                if (query.hasUserFormatOptions()) {
                    ...
                    for (AbstractColumn column : formats.getColumns()) {
                        newFormats.addPattern(new SimpleColumn(column.getId()),
                                formats.getPattern(column));
                    }
     ...
     * Algorithm: if the query has filter, grouping, pivoting or skipping
     * requirements the query is split as in the NONE case. If the query does
     * not have filter, grouping, pivoting or skipping the data source query
     * receives any sorting or pagination requirements and the completion query
     * receives any selection requirements.
     *
     * @param query
     *            The query to split.
     *
    ...
    private static QueryPair splitSortAndPagination(Query query) {
        ...
        if (query.hasFilter() || query.hasGroup() || query.hasPivot()) {
            ...
            // 2. Pagination should be performed in the data source query IFF
            // skipping
    ...
    /**
     * Splits the query for a data source with capabilities SELECT. Algorithm:
     * the data source query receives any select operation from the query,
     * however the completion query also receives selection to properly
